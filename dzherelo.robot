...
Підготувати клієнт для користувача
  ...
  Run Keyword If   '${username}' != 'dzherelo_viewer'   Login   ${username}
...
Створити тендер
  ...
  ...      ${ARGUMENTS[1]} ==  tender_data
    ...
    ${value_valueaddedtaxincluded}=          convert_dzherelo_string      ${value_valueaddedtaxincluded}
    ...
    ${auctionperiod_startdate}=        dzherelo_convertdate   ${auctionperiod_startdate}
    ...
    dzherelo.Додати предмети      ${items}
...
Додати предмети
    ...
    \   dzherelo.Додати предмет   ${items[${index}]}     ${index}
...
Завантажити документ
    ...
    dzherelo.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
    Click Element       xpath=//a[contains(@id, 'update_auction_btn')]
    Sleep   2
...
Задати запитання на тендер
  ...
  dzherelo.Задати питання  ${username}  ${tender_uaid}  ${question}
...
Оновити сторінку з тендером
    ...
    Go to   ${USERS.users['${ARGUMENTS[0]}'].homepage}
    dzherelo.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію про awards[0].status
    ...
    ${return_value}=   convert_dzherelo_string     ${return_value}
    [Return]  ${return_value}
...
Внести зміни в тендер
  ...
  ...  ELSE IF  '${field_name}' == 'dgfDecisionDate'  dzherelo_convertdate  ${field_value}
...
Отримати інформацію про tenderPeriod.startDate
  ...
  ${return_value}=   convert_dzherelo_date_to_iso_format   ${return_value}
...
Отримати інформацію із документа
  ...
  ${tender}=  dzherelo.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Отримати документ
    ...
    dzherelo_download_file   ${url}  ${file_name}  ${OUTPUT_DIR}
    [return]  ${file_name}
...
Подати цінову пропозицію
    ...
    Run Keyword If    ${presentsum}     dzherelo.Вказати цінову пропозицію   ${bid}
...
Завантажити фінансову ліцензію
    [Arguments]  ${username}  ${tender_uaid}  ${filepath}
    dzherelo.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}
...
Отримати інформацію із пропозиції
    ...
    ${bid}=   dzherelo.Отримати пропозицію  ${field}
